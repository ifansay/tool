...
"""
功能实现
1.扫描文件夹(.py当前路径)的文件(不包含文件夹),并将文件名、拓展名、创建时间、最后更新时间、大小写入excel
2.用户可在excel对文件打标签(特殊标签:删除)
3.再次执行脚本,按照标签对文件分类(删除标签扔到垃圾箱,其他文件按标签分类,无标签不动)
4.txt记录归档记录
归类方式
1.一键归类-->时间方式
2.拓展名归类
...
# path = sys.path[0]
# path = "D:/test"
path, code_file = os.path.split(os.path.realpath(__file__))  # code_file
os.chdir(path)
file_list, file_info = obtainFile(path, code_file)
print('当前归档目录为:%s' % path)
...
if arrange_file in file_list:
    ...
    if shutil_mode in ('1', '2'):
        ...
        writeExcel(path, info_dict, arrange_file)
